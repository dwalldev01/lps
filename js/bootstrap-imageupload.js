...
(function($) {
    ...
    $.fn.imageupload.defaultOptions = {
        allowedFormats: [ 'jpg', 'jpeg', 'png', 'gif', 'doc', 'docx', 'pdf', 'htm' ],
        maxWidth: 250,
        maxHeight: 250,
        maxFileSizeKb: 5048
    };
    ...
    function getImageThumbnailHtml(src) {
          ...
          if (~src.indexOf("data:application"))
          {
              var filename = document.getElementById('doc-file'); 
            var name1 =  filename.files.item(0).name;
              var altstring = name1;
 
          }else
          {
           var altstring = "Document";   
          }
      
       //  alert(src);
        return '<img src="' + 
        src + '" alt="'+altstring +'" class="thumbnail" style="max-width: ' + options.maxWidth + 'px; max-height: ' + options.maxHeight + 'px">';
    }
